...
@if ($message = Session::get('permit-uploaded'))
<div x-data='{ open: true }' class="fixed z-50 bottom-0 left-0 w-full p-4 md:w-1/2 md:top-0 md:bottom-auto md:right-0 md:p-8 md:left-auto xl:w-1/3 h-auto rounded">
    <div class="bg-white rounded p-4 flex items-center shadow-lg h-auto border-gray-200 border" x-show='open'>
        <div class=" mr-4 rounded-full p-2">
            <div class=" rounded-full p-1 border-2">
...
<div class="mx-auto py-6 sm:px-6 lg:px-32">
    <div class="px-4 py-6 sm:px-0">
        <div class="bg-grey-light lg:w-full mt-12 lg:mt-0">
            @if (Auth::user()->permit_approved == true)
            <div class="flex items-center justify-end mr-8 -mb-8">
                <a href="{{ route('products.create') }}" class="relative w-50 flex justify-center mt-8 py-2 px-4 border border-transparent text-md font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                    + New Product
                </a>
            </div>
            @else
            <p class="text-blue font-base ml-8 py-4 text-gray-700">Your permit is yet to be verified soon & then you can upload your products. A message will be sent to your inbox and your email when it is verified.</p>
            @endif
            <div class="py-8 px-5 md:px-8">
                @livewire('seller-products-table-view')
            </div>
        </div>
    </div>
</div>
...
<div class="mx-auto py-6 sm:px-6 lg:px-32">
    <div class="px-4 py-6 sm:px-0">
        <div class="bg-grey-light lg:w-full mt-12 lg:mt-0">
            ...
            <div class="py-8 px-5 md:px-8">
                @livewire('admin-products-table-view')
            </div>
        </div>
    </div>
</div>
